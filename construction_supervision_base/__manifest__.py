# -*- coding: utf-8 -*-
{
    ...
    'version': '18.0.6.0.0',  # 6.0.0: 契約工項加「整包費用項」旗標（金額走 xml_amount，不被寫髒的 unit_price 影響）與「比例基數」多選（基數限同層前置工項，故不需循環偵測）；整包項的實際金額改跟基數完成比例走。 5.14.0: 契約工項加「不計入契約金額」（預備單價項目），工程加「契約金額調整」——契約金額＝頂層工項合計＋調整累計。 5.13.0: 檔案總覽/資料夾加單檔下載鈕、勾選後可打包 zip
    # 5.4.0: 營造廠商移到基本資訊區
    # 5.3.0: 契約工項選取對話框
