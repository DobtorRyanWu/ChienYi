# -*- coding: utf-8 -*-
{
    ...
    #        補齊後兩式只差預約式獨有的 slip
    # 6.4.0: 修改「已經在使用中」的自主檢查類型時，存檔前跳 onchange 警告，
    #        提醒去確認既有紀錄是否需要一併調整（使用者要求）。
    #        🔴 這不是資料完整性的守門——既有紀錄本身不會被改壞（項目名稱、
    #        檢查標準、量測句型都是建立當下的快照，實測改到面目全非、四種狀態
    #        的紀錄匯出內容一字不變）。它擋的是**業務落差**：範本加了一項檢查、
    #        改了檢查標準之後，那些用舊範本做的檢查要不要補做、要不要重新判定，
    #        是監造人員要決定的事，系統負責讓他知道有這件事。
    #        ⚠️ 曾主張「只對會漂移的 4 個名稱欄位提示」，被使用者正確駁回——
    #        那是拿「資料會不會被竄改」去回答「業務上要不要處理」，答非所問。
    #        ⚠️ onchange 只在畫面操作時觸發；匯入與程式寫入不會跳。
    # 6.3.0: 刪整個自主檢查類型時，原本是 PostgreSQL 的 RESTRICT 先炸，使用者看到
    #        "update or delete on table self_inspection_type violates RESTRICT"
    #        這種原始 SQL 訊息（2026-09-14 實測）。補 @api.ondelete 守門，
    #        講清楚被幾筆檢查紀錄使用、並建議改用「停用」。
    #        段落／時機／量測區塊三個子表本來就有這種守門，只有類型本身漏了。
    # 6.2.0: 兩式自主檢查的檢查項目清單移除「從樣板選取」(type_item_id) 欄 ——
    #        使用者確認它是無效欄位：前台選了檢查類型就自動帶入全部預設項目、
    #        後台有「載入預設檢查項目」按鈕，沒有逐項挑的必要；而且業務上
    #        「只要發生自主檢查，每一項都必須填，沒檢查的也要填劃線記號(na)」，
    #        本來就不存在「只載入部分項目」。
    #        🔴 只動 list 顯示：模型欄位、_onchange_type_item_id、
    #        action_load_default_items 的寫入全部保留 —— 它是血緣
    #        (這一項來自哪個預設項目)，且 check_item 是快照不能改成 related
    #        (改檢查類型的項目名不可以回頭改掉歷史紀錄)。
    # 6.1.0: 依使用者裁示把 subcontractor_name（協力廠商）加回來，放在承攬廠商之下。
    #        4.7.0 以「兩式各 0 筆填值」為由移除是看錯了證據——紙本表頭本來就有
    #        這一格（79 份 docx 樣板每一份都有 inspection.subContractor），
    #        0 筆是因為系統沒地方填，不是因為不需要。純 Char、無 compute、
    #        無預設值；列印報表的承攬廠商由 colspan=3 改回兩欄，右半放協力廠商。
