...
> **ChienYi construction_portal v10 專用橋接說明（優先於下方規範）**
>
> 本檔案為 AI 撰寫 UI 時的設計參考。**不要直接輸出下方的色碼**，一律對映到 v10 的 `--wb-*` CSS 變數（定義於 `static/src/css/portal_mobile.css`）。
>
> **硬性約束**（優先於下方規範）：
> - 嚴禁使用 `--cy-*` 變數（v1.6.0 遺物，v10 已汰除）
> - 嚴禁引用 `portal_workbench.py` / `WorkbenchPortal` / `cy_base_layout` / `portal_mobile_app_shell` / `portal_pc.py` / `portal_admin.py` / `_get_accessible_projects()` / `_is_mobile()`
> - 路由一律在 `/my/construction/*`；controller 加在 `portal.py`，不新建 controller 檔案
> - 技術方案：QWeb server render + vanilla JS（不用 OWL、不用 jQuery）
> - Selection 欄位一律 `fields_get` 拉；Odoo 18 `<list>` 不是 `<tree>`
>
> **色碼對映原則**：品牌主色 → `var(--wb-primary)`；success/danger/warning → `var(--wb-success|danger|warning)`；heading 色 → `var(--wb-text-heading)`。字體不換品牌專用字體，沿用系統 stack；但可借用排版哲學（字重、字距、數字表格化）。
>
> **衝突順序**：本橋接段 → `addons/CLAUDE.md`（遷移指令）→ `PORTAL_DESIGN_SPEC.md`（欄位審計）→ 下方本體。前者優先。
