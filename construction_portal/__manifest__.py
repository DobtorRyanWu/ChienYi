# -*- coding: utf-8 -*-
{
    ...
    'version': '18.0.2.19.1',  # 2.19.1: 依裁示移除水位監測頁的「示範資料」橫幅（示範標示仍留在站名文字、後台緞帶與匯出檔警語）。 2.19.0: 水位監測頁增量——自訂區間查詢（日期+小時，上限 30 天）、區間摘要（最高/最低/平均/筆數）、逐筆資料表（最新在上、可捲動、超過警戒標色）、CSV 匯出（/water-level/export.csv，原始逐筆、含示範資料警語）與歷程圖 PNG 匯出；空圖改顯示最後一筆時間並提供改看更長區間的按鈕。 2.18.0: 前台兩式自主檢查詳情頁新增「匯出檢查表（Word）」——套進該檢查類型上傳的樣板（沒上傳則用系統預設的動態表格樣板）。套印失敗（樣板與檢查項目對不上）時不能變成 500 白頁：改成捕捉 UserError、帶訊息轉回詳情頁，並用 pre-wrap 完整顯示（訊息會逐段指出差在哪一項，很長）。 2.17.0: 依使用者裁示把自主檢查的「協力廠商」加回前台（2.12.0 當初隨模組一起移除）——兩式詳情頁顯示、預約式新增表單輸入框、controller 寫入。純文字、無 compute，所以留空就是留空、直接寫入（與承攬廠商的「有值才放進 vals」不同）。 2.16.1: 兩式自主檢查詳情頁的「量測記錄」段落整段被寫在 <t t-if="inspection.note"> 裡面 → 備註沒填時，即使該筆已填量測列也完全不顯示（資料在庫裡，只是渲染不出來）。改成獨立判斷 measure_line_ids，與備註無關。新增表單那兩處沒有這個問題。 2.16.0: 自主檢查前台支援「檢查時機（複選）」與「量測區塊」的填寫（設定端刻意只在後台）。檢查時機依所選檢查類型渲染勾選框；量測區塊依句型切段、在段與段之間插輸入框。⚠️ 版面兩個實測缺陷：(a) 量測區塊誤插進「檢查日期／檢查時機」那個 flex 列，變成第三欄 → 430px 手機三欄各剩約 130px、檢查時機的字被擠成一字一行；改成獨立一列並讓該列 flex-wrap。(b) 勾選框太小 → 整個 label 變可點區域（字 0.95rem、方框 20×20、min-height 42px）。位置依裁示放在「檢查項目」與「備註」之間（紙本上它就在檢查項目表格下方），兩個新增表單與兩個詳情頁共四處一致。 2.15.0: 通報單詳情頁的彙總（小計）列只顯示金額，不顯示數量單價。 2.14.0: 施工日誌詳情頁新增「營造業特定項目」區塊（唯讀顯示，對應後台同名頁籤）。2.13.0: 前台檔案管理改串「檔案總覽」——列表與上傳都直接讀寫 ir.attachment（不再建 supervision.document），篩選由 44 個文件分類 pills 改成資料夾下拉，上傳可指定既有資料夾（前台不提供建立資料夾）、分類由資料夾自動繼承；/construction/doc 的允許清單改判 supervision_project_id。2.12.0: 自主檢查移除協力廠商欄（無效欄位，DB 0 筆填值）——前台新增表單的輸入框、詳情頁顯示、controller 寫入全部清掉。2.11.0: 預約式自主檢查前台建立時，承攬廠商留白會送出空字串，而 Odoo 判斷 computed 欄位有沒有被明給是看 key 在不在、不看值真假 → 空字串一樣算明給，把「自動帶入工程營造廠商」整個擋掉；改成有值才放進 vals。2.10.0: 檢試驗詳情頁「進場日期」改為「實際進場日期」，並新增「預定進場日期」(配合工程會新版管制表)。2.8.0:/improve 的「請先填寫矯正措施」提前到動任何資料之前擋（原本排在最後一關，擋下來時照片已存、狀態已推進，補填重送會多一張重複照片）。2.7.0: 照片三個描述欄位統一 —— 新增 _post_photo_meta() 兩套欄位名都收(比照 _post_geo)，修掉「缺失/檢試驗/通報單三處表單有欄位但 controller 沒收、送出後靜默丟掉」；檢試驗表單移除重複的同名 description input；自主檢查新增表單改用共用片段。2.6.0: 前台照片座標欄位 —— 新增共用片段 cy_photo_geo_fields(定位鈕+手填)並套進日誌鎖定後追加/檢試驗/缺失三處表單；_defect_save_photos 支援 meta 座標；通報單新增「設定地點座標」表單與 /set-geo 路由；_post_geo() 統一吃 photo_latitude 與 latitude 兩套欄位名。2.5.0: 缺失防竄改 —— 已驗證/結案缺失前台不可刪照片、不可改寫改善內容(含模板同步隱藏刪除鈕)；缺失改善明細補充(enrich)補上 _require_write(原本零角色檢查，閱覽角色可 sudo 改資料)。2.4.0: 修前台刪除照片 MissingError(整個 transaction rollback → 報錯且沒刪掉) + 新增前台缺失照片刪除路由與縮圖刪除鈕。2.3.0: 前台缺失照片補同步到照片管理(_defect_save_photos 原本只建照片行、mixin 不觸發) + 既有資料一次性回填。2.2.1: M0.6 文件庫附件收 public=False + 帶權限下載端點 /construction/doc。2.2.0: 照片附件收 public=False + /construction/img。2.1.1: 老闆看全庫所有專案(不分狀態)
