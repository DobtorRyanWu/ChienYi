# -*- coding: utf-8 -*-
{
    ...
    'version': '18.0.1.3.1',  # 1.3.1: 工程案件表單的 change_limit_exempt 宣告移出它所控制的 group，比照同檔 change_order_count 的既有寫法。 1.3.0: 採購法 50% 管制新增豁免路徑（機關改依其他法源核准變更時使用）——勾選即必填緣由，核定後本工程之後的變更一律沿用並註明由第幾次啟用，被沿用時不得取消。 1.2.0: 發包工程費調整金額（工項不動的契約金額追加/追減，會落到頂層彙總明細）、採購法第 22 條 50% 累計變更管制、預備單價項目不計入契約金額。
