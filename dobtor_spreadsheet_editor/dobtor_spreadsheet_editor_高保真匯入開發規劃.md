...
| 4 CF/DV | 🟡 ~45% | §1.7 CF 解析 + §4.1 CF 編譯（CellIsRule+containsText+dxf S29、**colorScale 2/3色 S34**） | §4.1 dataBar/iconSet/duplicateValues/expression、§4.2 DV 編譯、§4.3 CF 視覺回歸 |
...
- [ ] type = `list`：`<formula1>`（清單 range 或 inline `"opt1,opt2,opt3"`）
- [ ] type = `whole` / `decimal` / `date` / `time` / `textLength`：operator + formula1 + formula2
...
- [ ] 日期 token：`yyyy`、`mm`、`dd`、`hh`、`mm`（分鐘 vs 月份判定）、`ss`、`AM/PM`
- [ ] 中華民國年（民國紀年 `e` token）
- [ ] 台灣常見 15 種自訂格式：
  - `#,##0` `#,##0.00` `#,##0.000`（工程計量）
  - `0.00%` `0.0%`（百分比）
...
#### 3.5 公式錯誤值（3 天）
- [ ] Cell `t="e"` + 錯誤值（`#NULL!` / `#DIV/0!` / `#VALUE!` / `#REF!` / `#NAME?` / `#NUM!` / `#N/A` / `#GETTING_DATA`）
- [ ] o-spreadsheet 錯誤值對應顯示
...
- [ ] `list` type → dropdown（inline 清單 vs range 清單）
- [ ] `whole` / `decimal` / `date` / `time` / `textLength` type + operator
- [ ] `custom` type → 公式驗證
