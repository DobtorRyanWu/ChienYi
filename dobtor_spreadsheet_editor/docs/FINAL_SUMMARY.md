# dobtor_spreadsheet_editor — 最終總結（Sprint 0-41）
...
| **MROUND / REPT / SIGN** | o-spreadsheet 18.0.48 未內建 → 走 cached（可比照 CHOOSE shim 補） |
...
**短期（小、有界）**
- MROUND/REPT/SIGN functionRegistry shim（同 CHOOSE 模式）
- autoFilter → o-spreadsheet filter（需先確認不破壞視覺）
- 日期格式碼 / 民國年（e token）顯示渲染（§2.3 剩餘）

**中期**
- chart 編輯回寫 xlsx（ChartAst → chartN.xml）
- R1C1 → A1 轉換（若遇 R1C1 模式檔）
- 大檔效能：契約詳細表 16 sheet / 6.5 萬公式的開啟時間量測與優化
