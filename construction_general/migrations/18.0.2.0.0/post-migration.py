...
def migrate(cr, version):
    ...
    # 1. 統一 check_type：將舊類型轉換為新類型
    cr.execute("""
        UPDATE general_defect_improvement
        SET check_type = 'construction'
        WHERE check_type IN ('construction', 'quality', 'environment');
    """)
    cr.execute("""
        UPDATE general_defect_improvement
        SET check_type = 'safety_env'
        WHERE check_type = 'safety';
    """)
    ...
    cr.execute("""
        ...
        SELECT DISTINCT
            ...
            1 as create_uid,
            1 as write_uid
        FROM general_defect_improvement
        WHERE project_id IS NOT NULL
          AND NOT EXISTS (
              SELECT 1
              FROM defect_improvement_prefix_config dipc
              WHERE dipc.project_id = general_defect_improvement.project_id
          );
    """)

    # 4. 計算並設定序號（按工程、日期、類型分組）
    cr.execute("""
        ...
        UPDATE general_defect_improvement gdi
        SET sequence_number = nr.seq_num
        FROM numbered_records nr
        WHERE gdi.id = nr.id;
    """)

    # 5. 觸發編號重新計算（透過 compute 欄位）
    # 這會在模組更新後自動執行
