# -*- coding: utf-8 -*-
{
    ...
    'version': '18.0.2.8.0',  # 2.8.0: 手填金額閘門補一道判準——契約工項標記為「整包費用項」者一律可手填，即使明細列的單價欄被寫髒（保險絲：上游 contract_change 18.0.2.0.0 起已不再污染 unit_price）。 2.7.0: 施工詳細表可選彙總項（選葉節點自動補齊祖先），金額改「有子列加總／無子列手填／葉節點數量×單價」三分支，結算金額只加總根列，新增明細預估合計與不符示警。
