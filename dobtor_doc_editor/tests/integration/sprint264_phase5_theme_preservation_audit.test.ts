/**
 * Sprint 264 — Phase 6 Phase 5 (07/08/09) fixture theme.xml 第十八層 audit
 *
 * 對齊 Sprint 262 writer。閾值 90% 對齊 Phase 5 audit。
 */
...
describe('Sprint 264 — Phase 6 Phase 5 (07/08/09) fixture theme.xml preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`theme SHA-256 對照：${EXPECTED_FIXTURE_COUNT} Phase 5 fixture 保留率 ≥ ${MIN_THEME_MATCH_RATE_PCT}%`, () => {
    ...
    console.log(`[sprint264] total=${total} match=${matchCount}/${total} (${matchRate.toFixed(1)}%) hasTheme=${hasThemeCount}/${total} totalColors=${totalColors} totalFonts=${totalFonts}`);
    for (const cat of PHASE5_CATEGORIES) {
      const stats = byCategory[cat];
      if (!stats) continue;
      const pct = stats.total > 0 ? (stats.match / stats.total) * 100 : 0;
      // eslint-disable-next-line no-console
      console.log(`[sprint264]   ${cat.padEnd(14)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) hasTheme=${stats.hasTheme} colors=${stats.colors} fonts=${stats.fonts}`);
    }
    for (const r of results.filter((x) => !x.match)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint264]   DIFF ${r.path}: hasTheme=${r.hasTheme} colors=${r.colorKeys} fonts=${r.fontKeys}`);
    }
