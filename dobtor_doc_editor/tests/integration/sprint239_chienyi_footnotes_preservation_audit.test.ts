/**
 * Sprint 239 — Phase 6 ChienYi 42 fixture Footnotes preservation audit (第十一層)
 *
 * Footnotes（OOXML §17.11、word/footnotes.xml）— Sprint 145 capture-only。
 * audit 驗 id / type / content text round-trip 一致。空 Map 為 trivially match。
 */
...
describe('Sprint 239 — Phase 6 ChienYi 42 fixture Footnotes preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`Footnotes SHA-256 對照：${EXPECTED_FIXTURE_COUNT} fixture 保留率 ≥ ${MIN_FOOTNOTE_MATCH_RATE_PCT}%`, () => {
    ...
    console.log(`[sprint239] total=${total} match=${matchCount}/${total} (${matchRate.toFixed(1)}%) totalFn=${totalFn} totalEn=${totalEn}`);
    for (const cat of CHIENYI_CATEGORIES) {
      const stats = byCategory[cat];
      if (!stats) continue;
      const pct = stats.total > 0 ? (stats.match / stats.total) * 100 : 0;
      // eslint-disable-next-line no-console
      console.log(`[sprint239]   ${cat.padEnd(20)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) fn=${stats.fn} en=${stats.en}`);
    }
    for (const r of results.filter((x) => !x.match)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint239]   DIFF ${r.path}: fn=${r.footnoteCount} en=${r.endnoteCount}`);
    }
