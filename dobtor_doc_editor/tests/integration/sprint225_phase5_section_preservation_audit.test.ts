/**
 * Sprint 225 — Phase 6 Phase 5 (07/08/09) fixture SectionProps preservation audit
 *
 * Sprint 223 ChienYi 42 達 42/42 100%（writer docGrid fix）/ Sprint 224
 * LibreOffice 288 達 239/288 (83.0%) 過閾值；本 sprint 補 Phase 5 18
 * fixture、完成三 corpus 六層矩陣。
 */
...
const MIN_SECTION_MATCH_RATE_PCT = 90;
...
describe('Sprint 225 — Phase 6 Phase 5 (07/08/09) fixture SectionProps preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`SectionProps SHA-256 對照：${EXPECTED_FIXTURE_COUNT} Phase 5 fixture section page-metadata 保留率 ≥ ${MIN_SECTION_MATCH_RATE_PCT}%`, () => {
    interface Result {
      path: string;
      category: string;
      sectionCount: number;
    ...
    console.log(
      `[sprint225] total=${total} section match=${matchCount}/${total} (${matchRate.toFixed(1)}%) ` +
    ...
    for (const cat of PHASE5_CATEGORIES) {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `[sprint225]   ${cat.padEnd(14)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) sections=${stats.sections}`,
      );
    }
    for (const r of results.filter((x) => !x.sectionMatch)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint225]   DIFF ${r.path}: sectionCount=${r.sectionCount}`);
    }
