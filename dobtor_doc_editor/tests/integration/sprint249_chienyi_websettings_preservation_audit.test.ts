/**
 * Sprint 249 — Phase 6 ChienYi 42 fixture DocumentWebSettings preservation audit (第十四層)
 *
 * Sprint 148 capture-only / 規畫書 §17.16 webSettings.xml。
 */
...
describe('Sprint 249 — Phase 6 ChienYi 42 fixture DocumentWebSettings preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`WebSettings SHA-256 對照：${EXPECTED_FIXTURE_COUNT} fixture 保留率 ≥ ${MIN_WEBSETTINGS_MATCH_RATE_PCT}%`, () => {
    ...
    console.log(`[sprint249] total=${total} webSettings match=${matchCount}/${total} (${matchRate.toFixed(1)}%) totalKeys=${totalKeys}`);
    for (const cat of CHIENYI_CATEGORIES) {
      const stats = byCategory[cat];
      if (!stats) continue;
      const pct = stats.total > 0 ? (stats.match / stats.total) * 100 : 0;
      // eslint-disable-next-line no-console
      console.log(`[sprint249]   ${cat.padEnd(20)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) keys=${stats.keys}`);
    }
    for (const r of results.filter((x) => !x.match).slice(0, 5)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint249]   DIFF ${r.path}: keys=${r.keys}`);
    }
