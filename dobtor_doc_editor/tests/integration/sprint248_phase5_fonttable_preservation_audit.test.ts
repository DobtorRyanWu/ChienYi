/**
 * Sprint 248 — Phase 6 Phase 5 (07/08/09) fixture FontTable preservation audit (第十三層)
 */
...
describe('Sprint 248 — Phase 6 Phase 5 (07/08/09) fixture FontTable preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`FontTable SHA-256 對照：${EXPECTED_FIXTURE_COUNT} Phase 5 fixture 保留率 ≥ ${MIN_FONT_MATCH_RATE_PCT}%`, () => {
    ...
    console.log(`[sprint248] total=${total} font match=${matchCount}/${total} (${matchRate.toFixed(1)}%) totalFonts=${totalFonts}`);
    for (const cat of PHASE5_CATEGORIES) {
      const stats = byCategory[cat];
      if (!stats) continue;
      const pct = stats.total > 0 ? (stats.match / stats.total) * 100 : 0;
      // eslint-disable-next-line no-console
      console.log(`[sprint248]   ${cat.padEnd(14)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) fonts=${stats.fonts}`);
    }
    for (const r of results.filter((x) => !x.match)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint248]   DIFF ${r.path}: fontCount=${r.fontCount}`);
    }
