/**
 * Sprint 238 — Phase 6 Phase 5 (07/08/09) fixture Comments preservation audit (第十層)
 *
 * 補完三 corpus 十層矩陣末端。
 */
...
describe('Sprint 238 — Phase 6 Phase 5 (07/08/09) fixture Comments preservation audit', () => {
  const fixtures = collectFixtures();
  const parser = new OoxmlParser();
  const writer = new OoxmlWriter();

  it(`Comments SHA-256 對照：${EXPECTED_FIXTURE_COUNT} Phase 5 fixture 保留率 ≥ ${MIN_COMMENT_MATCH_RATE_PCT}%`, () => {
    ...
    console.log(`[sprint238] total=${total} comment match=${matchCount}/${total} (${matchRate.toFixed(1)}%) totalComments=${totalComments}`);
    for (const cat of PHASE5_CATEGORIES) {
      const stats = byCategory[cat];
      if (!stats) continue;
      const pct = stats.total > 0 ? (stats.match / stats.total) * 100 : 0;
      // eslint-disable-next-line no-console
      console.log(`[sprint238]   ${cat.padEnd(14)}: ${stats.match}/${stats.total} (${pct.toFixed(1)}%) comments=${stats.comments}`);
    }
    for (const r of results.filter((x) => !x.match)) {
      // eslint-disable-next-line no-console
      console.log(`[sprint238]   DIFF ${r.path}: commentCount=${r.commentCount}`);
    }
