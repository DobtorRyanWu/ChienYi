...
## 1. 當前指標一覽（Sprint 204 結尾 — 規畫書 §5 checkbox sync sweep、加權平均 ~93-95% 商用級揭露）

| 指標 | 數值 |
|---|---|
| vitest | **1946 passed + 1 skipped**（`npm test` 全套口徑；Sprint 204 docs-only 0 變動）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
...
| Sprint audit doc | 204（最新 sprint204_planning_doc_checkbox_sync.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
