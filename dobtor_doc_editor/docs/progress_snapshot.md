...
## 1. 當前指標一覽（Sprint 188 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1737 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 188 +15：9 OoxmlWriter pBdr/shd/framePr + 6 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 48 次連續 byte-identical；Sprint 167-188 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 188（最新 sprint188_export_paraprops_advanced.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~45%（MVS + RunProps + ParagraphProps 完整） | Sprint 185 MVS + 186 RunProps + 187-188 ParagraphProps（含 pBdr/shd/framePr）；樣式繼承 / 多 section / numbering.xml / 表格 / 圖片 / 頁首頁尾 / Phase 5 子功能等留 Sprint 189+；預估 6-8 sprint 達 100% |
