...
## 1. 當前指標一覽（Sprint 182 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1621 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 182 +23：18 ChartParser + 5 Chart integration）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 42 次連續 byte-identical；Sprint 167-182 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts 皆 Strategy C 或 capture-only、42 fixture byte-identical） |
...
| Sprint audit doc | 182（最新 sprint182_chart_capture.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
