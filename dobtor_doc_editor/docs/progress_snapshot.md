...
## 1. 當前指標一覽（Sprint 190 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1780 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 190 +28：16 OoxmlWriter 表格 + 12 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、**第 50 次連續 byte-identical、半百里程碑**；Sprint 167-190 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 190（最新 sprint190_export_table.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~75%（文件流 + 樣式 + 表格） | Sprint 185-188 文件流+RunProps+ParagraphProps + 189 Styles.xml + **190 表格**（tblPr/tblGrid/trPr/tcPr/gridSpan/vMerge restart-continue/巢狀表格、空 cell 自動補 w:p）；多 section / numbering.xml / 圖片 / 頁首頁尾 / Phase 5 子功能等留 Sprint 191+；預估 4-5 sprint 達 100% |
