...
## 1. 當前指標一覽（Sprint 198 結尾 — Phase 7 邊緣相容性 audit 完成）

| 指標 | 數值 |
|---|---|
| vitest | **1912 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 198 +6：3 LibreOffice compat audit test + 3 自然增長）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
...
| Sprint audit doc | 198（最新 sprint198_libreoffice_compat.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 7 效能優化 | **~87%**（邊緣相容性 audit sub-item 已收）| cache 五連發 + LayoutCache + path coalescing + OffscreenCanvas probe（Sprint 60 GREEN）+ **Sprint 198 邊緣相容性 audit**（290 個 LibreOffice fixture 跑 OoxmlParser、**288/290 = 99.3% parse 成功 / 0 crash / 100% sections / 100% paragraphs / 96% styles**、2 個失敗皆為 LibreOffice 標示「故意畸形」的負向測試 case、皆 throw Error instance）；**剩餘 cluster**（每個 2-3 sprint、合計 ~8 sprint）：OffscreenCanvas worker render / Web Worker parse / 50+ 頁 fixture / 大檔 benchmark / WPS 來源 fixture audit。Sprint 197 final audit 判定：ChienYi 監造文件實際 20-50p、cache 五連發已達 7× warm path 加速、worker 改造收益與成本比 marginal、留長期 optional |
