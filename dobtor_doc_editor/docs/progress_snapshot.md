...
## 1. 當前指標一覽（Sprint 169 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1405 passed + 1 skipped**（Sprint 169 +20：frameGroup 14 + Paginator.framePr 6；Sprint 167 +14；Sprint 166 +10） |
| VR mean | **0.073191**（Sprint 65 promote、第 29 次連續 byte-identical；Sprint 167 textAlignment / Sprint 169 framePr 皆 opt-in Strategy C、42 fixture byte-identical） |
...
| Phase 4 Style Theme | 91% | Sprint 19 style merge;Sprint 130 §Phase 4.1 HSL;Sprint 131 §Phase 4.2 tblStylePr/tcPr;Sprint 132 §Phase 4.3 numberingFormatter（wire-up defer）;Sprint 133 §Phase 4.4 pBdr + shd + borderShading DRY;Sprint 134 §Phase 4.4 textAlignment + framePr capture;Sprint 137-139 numbering wire-up Strategy C;**Sprint 167 §Phase 4.4 textAlignment render wire-up（decision A part 1）**;**Sprint 168 framePr probe → user 選 opt-in 路徑**;**Sprint 169 §Phase 4.4 framePr 浮動段落框 layout wire-up（decision A part 2、cluster 1/2-3、frameGroup.ts + Paginator layFramedParagraphs、opt-in enableFramePr、Strategy C、VR byte-identical）**;framePr wrap=around 排除區留 Sprint 170 |
