...
## 1. 當前指標一覽（Sprint 195 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1866 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 195 +11：7 OoxmlWriter SmartArt+Chart + 4 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 55 次連續 byte-identical；Sprint 167-195 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 195（最新 sprint195_export_smartart_chart.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~99.8%（剩 watermark） | Sprint 185-194 + **195 SmartArt + Chart export**（writeInlineImageRun graphicData 分派、SmartArt diagrams/dataN.xml + Chart charts/chartN.xml 部件、Content_Types/rels 自動擴充、cat/val 稀疏 null 對位保留）；watermark 留 Sprint 196 optional（honest gap、ChienYi 監造文件實際少使用、不影響 95%+ docx 使用情境） |
