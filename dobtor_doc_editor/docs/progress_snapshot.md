...
## 1. 當前指標一覽（Sprint 194 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1855 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 194 +21：14 OoxmlWriter Phase 5 子功能 + 7 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 54 次連續 byte-identical；Sprint 167-194 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 194（最新 sprint194_export_phase5.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~99.5%（剩 watermark + SmartArt/Chart export） | Sprint 185-193 + **194 Phase 5 子功能 export**（5/7：OMML `<m:oMath>` 遞迴 + 追蹤修訂 `<w:ins>`/`<w:del>`+`<w:delText>` + 註解錨點 commentRangeStart/End/Reference + comments.xml 部件 + background `<w:background w:color>`）；7→8 必要 part 含 comments.xml；watermark + SmartArt/Chart 留 Sprint 195 收尾 |
