...
## 1. 當前指標一覽（Sprint 187 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1722 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 187 +23：13 OoxmlWriter ParagraphProps + 10 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 47 次連續 byte-identical；Sprint 167-187 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 187（最新 sprint187_export_paraprops.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~35%（MVS + RunProps + ParagraphProps） | Sprint 185 MVS 純文字 + 186 RunProps + 187 ParagraphProps（對齊/styleId/numbering/縮排/spacing/tabs/toggles/textAlignment/snapToGrid）；pBdr+shd+framePr / 樣式繼承 / 表格 / 圖片 / Phase 5 子功能等留 Sprint 188+；預估 7-9 sprint 達 100% |
| Phase 7 效能優化 | 84% | cache 五連發 + LayoutCache + path coalescing + OffscreenCanvas probe（Sprint 60 GREEN） |
| Phase 8 Template UI Builder | Phase 1 + 2.1 + Sprint A 已驗證 / 5-7 待 Sprint B-C / 2.2 未啟動 | ADR-022 落地（2026-05-19）、非 docx 匯入、工時 / VR mean 與 Phase 0-7 分開計算；Phase 1 視覺 + Phase 2.1 inline control 程式碼於 ADR-022 當日落地、2026-05-20 端到端驗證通過，詳見 [phase8_verification_2026-05-20.md](phase8_verification_2026-05-20.md)；**Sprint A（2026-05-23）收口 4 個 Phase 2 殘留 UI 缺口**：sub-nav 三分頁（儀表板/請求/設定）解封 + 預覽鈕接 `/dobtor_doc/template_preview` jinja2 sandbox 後端，92 backend test + 1699 vitest 全綠、詳見 [phase8_sprint_a_2026-05-23.md](phase8_sprint_a_2026-05-23.md)；缺口 5-7（縮圖 panel / 頁碼真換頁 / 縮放 fit）排 Sprint B-C；Phase 2.2 overlay 依 ADR-022 條件啟動、未動工 |
