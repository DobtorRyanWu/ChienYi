...
## 1. 當前指標一覽（Sprint 202 結尾 — Phase 7 大檔 perf baseline + Phase 8 平行推進）

| 指標 | 數值 |
|---|---|
| vitest | **1944 passed + 1 skipped**（`npm test` 全套口徑；Sprint 202 +3：sprint202_synthetic_50p_generator 3 test；+18 自 Sprint 201 後 Phase 8 平行 sprint L-Q 累積）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 58 次連續 byte-identical；Sprint 167-202 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical；Sprint 202 11_perf_synthetic_large 加入 PHASE5_FIXTURE_DIRS 排除集、不入 VR pipeline） |
...
| Sprint audit doc | 202（最新 sprint202_synthetic_50p_baseline.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
