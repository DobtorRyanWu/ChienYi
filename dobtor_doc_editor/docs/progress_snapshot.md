...
## 1. 當前指標一覽（Sprint 172 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1434 passed + 1 skipped**（Sprint 172 +10 WatermarkParser；Sprint 171 +14；Sprint 170 +5） |
| VR mean | **0.073191**（Sprint 65 promote、第 32 次連續 byte-identical；Sprint 167-172 textAlignment / framePr / background / watermark 皆 Strategy C 或 capture-only、42 fixture byte-identical） |
...
| Phase 5+（註腳 / 追蹤修訂 / OMML） | 進行中（5.6 背景 done） | Sprint 142 probe → user 2026-05-21 GO 全 6 子功能;**Sprint 171 §Phase 5.6「背景」完成**（`<w:background>` parse + render wire-up）;**Sprint 172 §Phase 5.6「浮水印」capture 完成**（header VML `<v:shape>` WatermarkParser、文字/圖片浮水印、capture-only、VR byte-identical）;浮水印 render wire-up 留 Sprint 173;5.4 追蹤修訂 / 5.5 註解 / 5.1 OMML / 5.2 SmartArt / 5.3 Charts 待續 |
