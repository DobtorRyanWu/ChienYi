...
## 1. 當前指標一覽（Sprint 192 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1817 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 192 +17：12 OoxmlWriter 圖片 + 5 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 52 次連續 byte-identical；Sprint 167-192 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 192（最新 sprint192_export_images.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~95%（文件流+樣式+表格+多 section+numbering+圖片） | Sprint 185-191 文件流+樣式+表格+多 section+numbering + **192 圖片/media**（InlineImage+FloatImage 降級、5 命名空間常數、EMU 換算、collectMedia data URL→bytes、Content_Types/rels 自動擴充含具名 Id 避鎖、PNG 簽名 zip 驗證、docPr 計數器多次 write 重置）；頁首頁尾 / Phase 5 子功能等留 Sprint 193+；預估 2-3 sprint 達 100% |
