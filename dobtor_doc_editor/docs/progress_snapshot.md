...
## 1. 當前指標一覽（Sprint 203 結尾 — Phase 7 vitest perf regression guard 上線）

| 指標 | 數值 |
|---|---|
| vitest | **1946 passed + 1 skipped**（`npm test` 全套口徑；Sprint 203 +2：sprint203_perf_regression_guard 2 test）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
...
| Sprint audit doc | 203（最新 sprint203_perf_regression_guard.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
