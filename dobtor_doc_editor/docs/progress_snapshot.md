...
## 1. 當前指標一覽（Sprint 193 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1834 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 193 +17：11 OoxmlWriter 頁首頁尾 + 6 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 53 次連續 byte-identical；Sprint 167-193 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 193（最新 sprint193_export_headerfooter.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~98%（文件流+樣式+表格+多 section+numbering+圖片+頁首尾） | Sprint 185-192 + **193 頁首頁尾**（headerN.xml/footerN.xml 部件、Content_Types/rels 自動擴充、sectPr headerReference+footerReference+titlePg、writeRefs 共用 default/first/even、writeBlock dispatcher reuse、xmlns:r 補宣告、CT_SectPr schema 順序）；Phase 5 子功能 export 留 Sprint 194+；預估 1-2 sprint 達 100% |
