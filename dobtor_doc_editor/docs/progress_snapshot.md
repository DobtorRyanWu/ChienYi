...
## 1. 當前指標一覽（Sprint 191 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1800 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 191 +20：10 OoxmlWriter 多 section+numbering + 10 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 51 次連續 byte-identical；Sprint 167-191 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 191（最新 sprint191_export_multisection_numbering.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
