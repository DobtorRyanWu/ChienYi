...
## 1. 當前指標一覽（Sprint 189 結尾）

| 指標 | 數值 |
|---|---|
| vitest | **1752 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 189 +15：8 OoxmlWriter Styles + 7 round-trip）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 49 次連續 byte-identical；Sprint 167-189 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 189（最新 sprint189_export_styles.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 6 Export 對稱性 | ~55%（MVS + RunProps + ParagraphProps + Styles.xml） | Sprint 185-188 文件流 + 樣式 + 189 Styles.xml（flat-equivalent：parser StyleResolver flatten 後 export 不需 docDefaults/basedOn 仍 round-trip 等價）；多 section / numbering.xml / 表格 / 圖片 / 頁首頁尾 / Phase 5 子功能等留 Sprint 190+；預估 5-7 sprint 達 100% |
