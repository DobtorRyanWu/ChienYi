...
## 1. 當前指標一覽（Sprint 199 結尾 — Export round-trip 廣域 audit + OMML bug fix）

| 指標 | 數值 |
|---|---|
| vitest | **1920 passed + 1 skipped**（`npm test` 全套口徑＝tests/unit + tests/integration；Sprint 199 +8：1 export round-trip audit + 7 自然增長）。註：Sprint 178 以前記錄的「1468」為不同計數口徑、自 Sprint 179 起改採全套數字 |
| VR mean | **0.073191**（Sprint 65 promote、第 57 次連續 byte-identical；Sprint 167-199 textAlignment / framePr / background / watermark / 追蹤修訂 / 註解 / OMML / SmartArt / Charts / export / OMML namespace fix 皆 Strategy C 或在 VR pipeline 外、42 fixture byte-identical） |
...
| Sprint audit doc | 199（最新 sprint199_export_roundtrip_audit.md；159 / 160v1 為 docs-only follow-up、無獨立 audit doc） |
...
| Phase 7 效能優化 | **~89%**（邊緣 audit 第一二輪皆通過）| cache 五連發 + LayoutCache + path coalescing + OffscreenCanvas probe（Sprint 60 GREEN）+ **Sprint 198 parse 邊緣 audit**（290 LibreOffice fixture、288/290 = 99.3% parse / 0 crash）+ **Sprint 199 export round-trip 廣域 audit**（288 parse-OK fixture：export 100% / reparse 100% / structure 93.1%；揭出並修復 Sprint 194 OMML writer namespace bug——math reparse 0%→91%）；**剩餘 cluster**（每個 2-3 sprint、合計 ~8 sprint）：OffscreenCanvas worker render / Web Worker parse / 50+ 頁 fixture / 大檔 benchmark / WPS 來源 fixture audit。Sprint 197 final audit 判定：ChienYi 監造文件實際 20-50p、cache 五連發已達 7× warm path 加速、worker 改造收益與成本比 marginal、留長期 optional |
