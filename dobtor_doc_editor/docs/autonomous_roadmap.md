...
| 168 | 2026-05-21 | 0.073191（未跑、純 docs）| **`<w:framePr>` wire-up 可行性 probe（decision A part 2）**。probe 3 步揭 framePr ≠ Sprint 167 textAlignment：framePr 是 layout-changing 浮動特性（段落移出正常流）、**無 Strategy C byte-identical 路徑**（框段落必移動才是正解）；唯一 fixture（06_template 檢試驗管制樣板、framePr 1/42×2）golden 由 LibreOffice 產生、framePr 保真度未知 → VR 驗證依賴 decision B（goldens 重生）；完整實作（連續框段落合併 + 子排版 + 錨點定位 + 排除區 + 跨頁）2-3 sprint、非單 PR-size。給 3 個 sequencing 選項（1 opt-in flag 實作 / 2 排 decision B 後 / 3 立即實作）→ **停下請 user 拍板**。§5 L514 維持 `[ ]`（probe DEFER 不打 `[x]`）。docs-only、0 行 production code。doc: [sprint168_framepr_wireup_probe.md](sprint168_framepr_wireup_probe.md) | 紀律 #22（probe 揭 mental model「textAlignment/framePr 同類」vs 實況差距）、#18（framePr 非 PR-size、scope 重分類走 user 認可）、#1.b（Sprint 136 翻車教訓：layout-changing 在 golden 未對齊前實作高風險）、#8（架構發現也是 sprint 產出）|
| 169 | 2026-05-21 | **0.073191（已跑、byte-identical 第 29 連）**| **`<w:framePr>` 浮動段落框 layout wire-up（decision A part 2、user 選 opt-in flag 路徑、cluster 1/2-3）**。`LayoutOptions.enableFramePr` opt-in flag（+15 行）+ 新檔 `frameGroup.ts`（+57 行純函式：isFramedParagraph / framePrEqual / frameGroupLength）+ Paginator block loop wire-up（paginate + layoutSectionInto 兩處）+ `layFramedParagraphs`（+約 140 行：連續同 framePr 段落合併為 frame、buildParagraph/breakParagraph 子排版、vAnchor(text/margin/page)+y 偏移定垂直、computeFrameX(xAlign/x+hAnchor)定水平、emit 絕對座標 LinePageEntry、vAnchor=text 採 topAndBottom-like 保留垂直空間）。**Strategy C**：enableFramePr 關 → block loop 永走原 layParagraph → byte-identical；VR pipeline 不傳 flag → 42 fixture VR 不變、framePr fixture 06_template mean 0.0263 未動。+20 test（frameGroup 14 + Paginator.framePr 6）、vitest 1385→1405。`tsc` 2 pre-existing error 無新增。frontend bundle 不重建（Paginator 非 production path、同 Sprint 162/167）。§5 L514 維持 `[ ]`（cluster 進行中）。doc: [sprint169_framepr_float_layout.md](sprint169_framepr_float_layout.md) | 紀律 #22（Sprint 168 probe 已建 mental model、本 sprint 直接實作無翻車）、#1.b/Strategy C（opt-in、預設走原路徑 byte-identical）、#1.a（改 layout 跑全 VR 第 29 連）、#14（framePr 判別純函式聚 frameGroup.ts）、#18（cluster 切分 Sprint 169 骨幹 / 170 排除區、框內 floatImage/numbering/跨頁/page·margin anchor 皆 scope-down honest declare）|
| 170 | 2026-05-21 | **0.073191（已跑、byte-identical 第 30 連）**| **`<w:framePr>` wrap 模式分派 + 側繞排除區（framePr cluster 2/2-3、decision A 收尾）**。`layFramedParagraphs` 垂直空間策略改依 `framePr.wrap` 三向分派（+約 25 行）：`around`/`tight`/`through`/未設 + 顯式 framePr.width + 旁留得下內文 → 註冊 `activeFloats` 排除區（複用 Sprint 6 wrapSquare 機制）、currentY 不推進、後續內文 per-line 側繞；`notBeside` → 保留垂直空間（Sprint 169 行為）；`none` → 純浮動。具名常數 FRAME_DEFAULT_HSPACE_PT=6 / FRAME_MIN_WRAP_TEXT_PT=72。**Strategy C**：opt-in `enableFramePr` 不變、側繞僅顯式 width 框（auto-width 退回保留空間、自然寬度 sizing 依賴 decision B golden）→ 唯一 framePr fixture（auto-width）與 Sprint 169 一致、VR pipeline 不傳 flag → 42 fixture byte-identical。+5 test、vitest 1405→1410。`tsc` 2 pre-existing error 無新增。§5 L514 `[ ]`→`[x]`（framePr layout wire-up 收尾）。**決策 A（textAlignment Sprint 167 + framePr Sprint 169-170）完成**。doc: [sprint170_framepr_wrap_exclusion.md](sprint170_framepr_wrap_exclusion.md) | 紀律 #1.b/Strategy C（opt-in + 側繞僅顯式 width、off/auto-width byte-identical）、#1.a（改 layout 跑全 VR 第 30 連）、#14 DRY（複用 activeFloats/exclusionAtY/purgeStaleFloats）、#2（具名常數）、#18（auto-width 側繞/框跨頁/page·margin anchor/xAlign 完整 → Sprint 171 optional）|
| 171 | 2026-05-21 | **0.073191（已跑、byte-identical 第 31 連）**| **`<w:background>` 文件背景 parse + render wire-up（Phase 5.6「背景」、決策 C）**。新型別 `DocumentBackground` + `DocumentNode.background?`（types +22 行）+ 新檔 `background/BackgroundParser.ts`（+88 行：w:color 6-hex 驗證+大寫正規化 / w:themeColor capture raw / 紀律 #21 空不掛）+ OoxmlParser Step 8.4 + `CanvasRenderer.pageBackgroundColor` 選項（預設 'FFFFFF'）+ VR pipeline entry `paintPage` 端到端接線（讀 documentNode.background.color）。**Strategy C**：42 fixture 全無 `<w:background>` → background undefined → renderer 用 DEFAULTS 白底 → byte-identical。synthetic XML 驗 parser（user 同意、tests/fixtures 無 Phase 5 樣本）。+14 test（BackgroundParser 12 + CanvasRenderer 2）、vitest 1410→1424。`tsc` 2 pre-existing error 無新增。frontend bundle 重建（OoxmlParser 在依賴樹內、production mapper 不消費 background、僅保產物同步）。scope-down：themeColor→hex 解析 / VML 圖片背景 / header VML 浮水印 shape → 後續。doc: [sprint171_background_parse_render.md](sprint171_background_parse_render.md) | 紀律 #1.b/Strategy C（pageBackgroundColor 預設白、無 background → byte-identical 第 31 連）、#1.a（改 render 跑全 VR）、#21（background optional 不掛空 key）、#14（BackgroundParser 自成 background/ 子目錄）、#18 scope-down、#8/決策 C（synthetic XML 驗 parser）|
| 172 | 2026-05-21 | **0.073191（已跑、byte-identical 第 32 連）**| **header VML 浮水印 shape capture（Phase 5.6「浮水印」、capture-only、決策 C）**。新型別 `DocumentWatermark` + `DocumentNode.watermark?`（types +34 行）+ 新檔 `watermark/WatermarkParser.ts`（+147 行：掃 header `getElementsByTagName('v:shape')`——`<v:textpath string>` 非空 → 文字浮水印 text/font(去引號)/rotation；shape id 含 "watermark" + `<v:imagedata r:id>` → 圖片浮水印 imageRId；偵測取捨：textpath 不要求 id、圖片要求 id 避免誤判 logo）+ OoxmlParser Step 8.5 `collectWatermark`（掃所有 header part 取第一個浮水印）。capture-only、layout/render 0 變更 → 42 fixture VR byte-identical。synthetic XML 驗 parser（user 同意）。+10 test、vitest 1424→1434。`tsc` 2 pre-existing error 無新增。frontend bundle 重建。scope-down：capture-only（render 留 Sprint 173）、第一個 shape、不解析透明度/尺寸。doc: [sprint172_watermark_capture.md](sprint172_watermark_capture.md) | 紀律 #1.a（capture-only layout/render 0 變更、VR byte-identical 第 32 連、同 Sprint 145-153 archetype）、#21（watermark optional 不掛空 key）、#14（WatermarkParser 自成 watermark/ 子目錄）、#18 scope-down、#8/決策 C（synthetic XML 驗 parser）|
| 173 | 2026-05-21 | **0.073191（已跑、byte-identical 第 33 連）**| **浮水印 render wire-up（Phase 5.6 浮水印收尾、決策 C）**。`CanvasRenderOptions.watermark` opt-in（watermark 無預設值、DEFAULTS 型別改 `Required<Omit<…,'watermark'>>`、class 另存 `private watermark?`）+ `CanvasRenderer.renderWatermark`（+37 行：文字浮水印——EstimateMetrics 量測反推字級使文字寬約頁寬 70% 上限 130pt、save/translate(頁心)/rotate(rotation°→rad) 旋轉、fillText 淺灰 'C8C8C8' 置中、繪於背景上內容下）+ VR pipeline entry 端到端接線。圖片浮水印 render no-op（需 shape 尺寸、留後續）。具名常數 ×4。**Strategy C**：無浮水印 → renderWatermark no-op → 無 save/translate/rotate → 42 fixture VR byte-identical。+5 test、vitest 1434→1439。`tsc` 2 pre-existing error 無新增。frontend bundle 不重建（CanvasRenderer 非 frontend 依賴樹）。**Phase 5.6「浮水印 + 背景」三 sprint 收尾**（171 背景 / 172 浮水印 capture / 173 浮水印 render）。doc: [sprint173_watermark_render.md](sprint173_watermark_render.md) | 紀律 #1.b/Strategy C（watermark opt-in、無浮水印 no-op byte-identical 第 33 連）、#1.a（改 render 跑全 VR）、#14 DRY（旋轉繪製複用 Sprint 34 save/translate/rotate）、#2（4 具名常數）、#18 scope-down（圖片浮水印 render / 淺灰近似 alpha / 字級估算）|
| 174 | 2026-05-21 | **0.073191（已跑、byte-identical 第 34 連）**| **`<w:ins>` / `<w:del>` 追蹤修訂 capture（Phase 5.4、capture-only、決策 C）**。新型別 `RunRevision`（type ins/del + author/date/id）+ `RunNode.revision?`（types +24 行、同 `hyperlink` optional metadata 模式）+ ParagraphParser：`parseRun` 的 `w:t` case 加 `w:delText`（+2 行）、段落子元素 loop 加 `w:ins`/`w:del` case（+約 20 行：parseRevision 解 author/date/id、展平 `effectiveChildren` 的 `<w:r>`、parseRun 後對 run 節點掛 revision）。probe 確認 0/42 fixture 含 ins/del → 新 case path 0 觸發 → 42 fixture VR byte-identical。synthetic XML 驗 parser（user 同意）。+6 test、vitest 1439→1445。`tsc` 2 pre-existing error 無新增。frontend bundle 重建。scope-down：capture-only（render 留 Sprint 175）、只處理直接 `<w:r>`、巢狀 ins/del / move / 屬性修訂留後續。doc: [sprint174_track_changes_capture.md](sprint174_track_changes_capture.md) | 紀律 #1.a（capture-only 0-coverage、VR byte-identical 第 34 連）、#22（probe 確認 ins/del 無 case + 0 fixture）、#21（revision optional、id 非數字不掛）、#14（revision 比照 hyperlink metadata 模式）、#18 scope-down、#8/決策 C（synthetic XML 驗 parser）|
| 175 | 2026-05-22 | **0.073191（已跑、byte-identical 第 35 連）**| **追蹤修訂 render wire-up（Phase 5.4 收尾、決策 C）**。`Box.revision?`（layout/types +2 行）+ BoxBuilder 透傳 `run.revision`→`pushTextAsBoxes`→`boxOf`（+約 6 行、紀律 #21 無值不掛）+ CanvasRenderer `renderBox` 裝飾段加追蹤修訂分支（+14 行：`revision.type==='ins'` 畫底線 baseline+fontSize×0.15、`'del'` 畫刪除線 baseline−fontSize×0.3、run 已有對應裝飾不重畫；刪除文字仍 fillText、markup view）。**Strategy C**：0/42 fixture 含修訂 → Box.revision 皆 undefined → renderBox 新分支 0 觸發 → 42 fixture VR byte-identical。+5 test、vitest 1445→1450。`tsc` 2 pre-existing error 無新增。frontend bundle 不重建（CanvasRenderer/BoxBuilder 非 frontend 依賴樹）。scope-down：per-author 修訂色 / 刪除隱藏（accept-reject）/ mapper 路徑留後續。**Phase 5.4「追蹤修訂」二 sprint 收尾**（174 capture / 175 render）。doc: [sprint175_track_changes_render.md](sprint175_track_changes_render.md) | 紀律 #1.b/Strategy C（revision 只真有修訂改輸出、0 fixture byte-identical 第 35 連）、#1.a（改 layout+render 跑全 VR）、#14 DRY（revision 透傳比照 hyperlink、裝飾複用 underline/strike drawLine）、#21（Box.revision 無值不掛）、#18 scope-down |
| 176 | 2026-05-22 | **0.073191（已跑、byte-identical 第 36 連）**| **comments.xml 註解 capture（Phase 5.5、capture-only、決策 C）**。新型別 `CommentContent`（id/author/date/initials/content）+ `DocumentNode.comments: Map`（types +22 行、比照 footnotes/endnotes 非 optional Map-valued part）+ 新檔 `comments/CommentsParser.ts`（+105 行：走訪 `<w:comment>`、解 id/author/date/initials、重用 `DocumentParser.parseBodyContent` 解內容、完全比照 Sprint 145 FootnotesParser）+ OoxmlParser `REL_TYPE_COMMENTS` + Step 6.5b `collectComments` + DocumentParser lite 建構子補 `comments: new Map()`。capture-only、layout/render 不消費 → 42 fixture VR byte-identical。synthetic XML 驗 parser（user 同意）。+9 test、vitest 1450→1459。`tsc` 2 pre-existing error 無新增。frontend bundle 重建。scope-down：commentRange 錨點 wire-up + 右側 panel render 留後續。**決策 C 可控部分（5.4 追蹤修訂 + 5.5 註解 + 5.6 浮水印背景）全數完成**。doc: [sprint176_comments_capture.md](sprint176_comments_capture.md) | 紀律 #1.a（capture-only、VR byte-identical 第 36 連、footnotes archetype）、#14（CommentsParser 比照 FootnotesParser、comments 比照 footnotes Map part）、#21（author/date/initials 缺不掛）、#18 scope-down、#8/決策 C（synthetic XML 驗 parser）|
