...
> **⚠️ 2026-08-25 更新：本系統同時支援兩種方向，這份文件寫的是其中一種。**
>
> | 方向 | 誰主動 | 用在哪 | 設定在哪 |
> |---|---|---|---|
> | **Push**（本文件） | 設備打 `POST /water-level/ingest` | 設備能主動連外時 | 設備上的「設備碼 + 上報金鑰」 |
> | **Pull** | Odoo 去對方系統撈 | 資料在合作方的資料庫／API 時 | 「品質安全 → 水位監測 → 資料來源」 |
>
> Pull 模式的欄位對應、時區、單位都在來源設定裡，對方不必為我們改欄位名。
> 要跟資料提供方確認哪些事，見 `SOURCE_SPEC_REQUEST.md`。
>
> **兩種方向下「自動索取補送」都成立**，只是取得的手段不同：
> push 是在回應裡請設備重送；pull 是我們直接向設備的資料庫重新索取同一段時間範圍
> （每次還會往回多撈一小時）。對客戶而言結果一樣——缺的自動補回來，不需要人介入。
> 兩者都靠 `(監測站, 時間)` 與 `(監測站, 序號)` 唯一鍵去重，重複送不會產生第二筆。
>
> **來源沒有流水號時**，缺號偵測會自動退回「時間模式」——用「沉默超過取樣間隔數倍」
> 找缺口。這把尺分不出「對方本來就沒量」與「資料在路上掉了」，所以能要到流水號還是要。
